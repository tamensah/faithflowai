...
- PR #17 merged the in-place setup controls and guide correction into `develop` after GitHub validation and both Vercel preview checks passed. Neon Function deployment 22 contains the matching API endpoint; `/health` and `/ready` each returned 200. The function retained all 12 existing environment entries. Using Vercel's preview access, the `develop` web `/guide` and admin `/sign-in` returned 200. The edit action itself still needs an authenticated browser test.
- PR #20 merged the global Church slug constraint, no-US-default migration, first Church slug suggestion, workspace-to-Organization name prefill, and clearer global church terminology into `develop`. GitHub Validate and both Vercel checks passed. Both migrations passed first on a disposable Neon branch (including a cross-organization duplicate-slug rejection); the branch was deleted. Both migrations then applied to Neon `develop`: two finished records, one global `Church_slug_key` index, and no `Church.countryCode` database default. Neon Function deployment 23 completed on Node 24; `/health` and `/ready` returned 200. Protected `develop` web home and guide and admin sign-in returned 200 through Vercel's authenticated check. The rendered web copy includes the fictional ChurchTrack Organization and explicitly says regional nesting is not yet available. This is a staging deployment check, not a fresh customer onboarding or authenticated edit test.
...
4. Repeat a fresh real-browser onboarding on deployment 23: confirm the workspace name prefills the in-app Organization, the first Church's slug follows its name until manually edited, country is explicitly selected, and a second operating Church can be created. Verify key admin modules and the member portal. The previous marketing → Google sign-in → church creation → Growth trial checkout passed before this deployment.
5. Design and test headquarters/regional oversight, delegated roles, and international timezone setup before claiming full multi-site readiness. Audit public feature, pricing, and privacy copy against deployed behavior and provider contracts before inviting external early adopters.
6. Promote `develop` to `main` only after the staging gates pass.
