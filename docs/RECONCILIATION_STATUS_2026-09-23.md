...
- The user's next admin sign-in landed on an access-restricted page because the separate admin origin had no active Clerk organization. The protected gate now selects a sole organization automatically and offers an organization switcher when selection or access verification fails. Both frontends clear their API query cache when the active organization changes. An authenticated browser retest is still required after the preview deployment.
- A fresh Incognito admin sign-in selected `TexaAlpha` but still showed access restricted. Clerk has one user and one organization, and that user has an `ADMIN` staff membership in Neon. The admin tRPC client omitted the API's SuperJSON transformer, so successful responses arrived as `{ json: { isStaff: true } }` while the gate read `isStaff` at the top level. The web client already supplied the transformer; the admin client now does too. An authenticated browser retest is required after this fix deploys.
...
4. Complete the authenticated admin dashboard and member portal retest in a real browser. Marketing → Google sign-in → church creation → Growth trial checkout passed; the admin response-decoding fix still needs retesting after the preview deployment.
